...
benthics<-read.table("2_outclean_benthics_2022_07_15_new.txt",header=TRUE, sep="\t", stringsAsFactors = FALSE)
names(benthics) <-gsub("X","",names(benthics))
benthics=benthics[,order(as.numeric(names(benthics)))]

rarecurve(t(benthics), sample=min(rowSums(t(benthics))), xlab="counts", ylab="benthic families")

col.sums.ben=as.data.frame(sort(colSums(benthics)))

#taxonomic filter: taxa should be in at least 3 samples with a minimum of 10 counts
benthics.ct=chooseTaxa(t(benthics), max.abun = 10, noccur=3)

########################

#resampling
specseq=as.data.frame(t(benthics.ct)) 
...
png(paste0("resampled_benthic_famnumber_totalspecies_sampleeffort",nsampleff,"_plot.png"), width=480,height=480)
par(mar=c(8,4,3,1),las=2)
with(totspec,plot(Nspecies~T, main="number of species per sample"))
dev.off()

# save processed data
write.table(totspec, paste0("resampled_benthic_famnumber_totalspecies_sampleeffort",nsampleff,".txt"), row.names=FALSE, sep="\t")	
write.table(totfam, paste0("resampled_benthic_famnumber_totalfamilies_sampleeffort",nsampleff,".txt"), row.names=FALSE, sep="\t")	
...
pdf(paste0("resampled_benthic_famnumber",nsampleff,"_aggregated.pdf"))
par(mar=c(8,4,3,1),las=2)
...
# save processed data
write.table(speciesfamiliesdf_totfam, paste0("resampled_benthic_famnumber_",nsampleff,"_aggregated.txt"), sep="\t", row.names=FALSE)
...
# export data
write.table(t(ordidf), paste0("resampled_benthic_famnumber_sampleeffort",nsampleff,"_aggregated_pcainput.txt"), sep="\t")
...
png(paste0("resampled_benthic_famnumber_sampleeffort",nsampleff,"_aggregated_comparison.png"), width=480,height=480)
par(mar=c(8,4,3,1),las=2)
